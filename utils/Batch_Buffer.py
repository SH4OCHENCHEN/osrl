...
import pickle
import random
from DMBPSAFE1.Utils import Batch_Class
...
class batch_buffer(object):
    def __init__(self, env_name0, device, buffer_mode='normal', buffer_normalization=False,
        ...
        if env_name == 'OfflineCarGoal1':
            dataset_path = f'datasets/{env_name}-v0.pkl'
        elif env_name == 'OfflinePointGoal1':
            dataset_path = f'datasets/{env_name}-v0.pkl'
        elif env_name == 'OfflineCarRun':
            dataset_path = f'datasets/{env_name}-v0.pkl'
        else:
            dataset_path = f'datasets/{env_name}-v0.pkl'

        proj_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        file_path = os.path.join(proj_dir, dataset_path)

        with open(file_path, 'rb') as f:
            trajectories = pickle.load(f)
            logger.info(f"{env_name} has been loaded!")

        states, actions, rewards, costs, traj_lens, returns = [], [], [], [], [], []
        ...
        if buffer_normalization:
            print(f'Buffer Information: {env_name} with Normalization on State')
        else:
            print(f'Buffer Information: {env_name} without Normalization')
